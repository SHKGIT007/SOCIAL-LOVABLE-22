...
const validateSocialAccountCredentialsUpdate = [
  ...
  body("app_secret").notEmpty().withMessage("App Secret is required"),
    ...
    .custom((value, { req }) => {
      if (req.files && req.files.image) {
        ...
        if (!allowedImageTypes.includes(req.files.image.mimetype)) {
          throw new Error(
            "Image must be a valid image file (jpeg, png, jpg, gif)"
          );
        }
      }
      return true;
    ...
    .custom((value, { req }) => {
      if (req.files && req.files.video) {
        ...
        if (!allowedVideoTypes.includes(req.files.video.mimetype)) {
          throw new Error(
            "Video must be a valid video file (mp4, avi, mov, mpeg)"
          );
        }
      }
      return true;
