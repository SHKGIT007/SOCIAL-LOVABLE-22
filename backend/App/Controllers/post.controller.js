...
const { checkContentModeration, moderateGeneratedContent } = require("../Utils/Services/contentModeration.service");
...
const generateAIPost = asyncHandler(async (req, res) => {
  ...
  // ðŸ”¥ Check AI post limit (AI posts used >= snapshotted limit)
  if (subscription.ai_posts_used >= subscription.ai_posts) {
    // ðŸ”¥ Automatically mark subscription as inactive
    await Subscription.update(
      { status: "inactive" },
      { where: { id: subscription.id } }
    );
  ...
  // âœ… Generate AI post content & image
  ...
  // ðŸ”¥ Increment AI posts used count
  ...
  // ðŸ”” Check AI Limit Thresholds (50%, 70%, 90%, 100%)
  ...
  // ðŸ”¥ Check if limit reached after increment and mark as inactive
...
const approvePost = asyncHandler(async (req, res) => {
  ...
  // âœ… Check scheduled time (only if scheduled_at exists)
  ...
  // âœ… Update review_status in database
