...
const SystemSettings = () => {
  const [activeTab, setActiveTab] = useState<
    "ai-provider" | "cloudinary" | "google-oauth"
  >("ai-provider");
        ...
        content: (
          <div>
            <p className="mb-3 text-sm text-gray-600">
              Please select an AI Provider Type from the dropdown above to see
              specific instructions for that provider.
            </p>
            <p className="text-sm text-gray-600">
              Available providers: <strong>Groq</strong>,{" "}
              <strong>OpenAI</strong>, <strong>Anyscale</strong>, and{" "}
              <strong>Other</strong>.
            </p>
          </div>
        ),
    ...
    switch (providerType) {
        ...
        return {
          ...
          content: (
            <ol className="list-decimal list-inside space-y-2 text-sm text-gray-700">
              <li>
                <strong>Sign up:</strong> Create account at{" "}
                <a
              ...
              <li>
                <strong>Get API Key:</strong> Go to <strong>API Keys</strong> →{" "}
                <strong>"Create API Key"</strong> → Name it → Click{" "}
              ...
              <li>
                ...
                <ul className="list-disc list-inside ml-4 mt-1">
                  ...
                  <li>
                    <strong>API Key:</strong> Copy the key (starts with{" "}
                    <code className="bg-gray-100 px-1 rounded">gsk_</code>) and
                    paste below
                  </li>
                </ul>
              </li>
        ...
        return {
          ...
          content: (
            <ol className="list-decimal list-inside space-y-2 text-sm text-gray-700">
              ...
              <li>
                <strong>Add payment:</strong> Go to <strong>Settings</strong> →{" "}
                <strong>Billing</strong> and add payment method (required)
              </li>
              <li>
                <strong>Get API Key:</strong> Go to <strong>API Keys</strong> →{" "}
                <strong>"Create new secret key"</strong> → Name it →{" "}
                <strong>"Create"</strong>
                <p className="text-xs text-red-600 mt-1 ml-4">
                  ⚠️ Copy immediately - key won't be shown again!
                </p>
              </li>
              <li>
                <strong>Copy credentials:</strong>
                <ul className="list-disc list-inside ml-4 mt-1">
                  ...
                  <li>
                    <strong>API Key:</strong> Paste the key (starts with{" "}
                    <code className="bg-gray-100 px-1 rounded">sk-</code>)
                  </li>
                </ul>
              </li>
            </ol>
        ...
        return {
          ...
          content: (
            <ol className="list-decimal list-inside space-y-2 text-sm text-gray-700">
              ...
              <li>
                <strong>Get API Key:</strong> Go to <strong>Settings</strong> →{" "}
                <strong>API Keys</strong> → <strong>"Create API Key"</strong> →
                Name it → Copy the key
              </li>
              <li>
                <strong>Copy credentials:</strong>
                <ul className="list-disc list-inside ml-4 mt-1">
                  ...
                  <li>
                    <strong>API Key:</strong> Paste the generated key below
                  </li>
                </ul>
              </li>
            </ol>
      ...
      case "other":
        return {
          ...
          content: (
            <ol className="list-decimal list-inside space-y-2 text-sm text-gray-700">
              <li>
                <strong>Get credentials:</strong> Sign up with your provider and
                get API key from their dashboard
              </li>
              <li>
                <strong>Find API endpoint:</strong> Check provider docs for chat
                completions endpoint (usually:{" "}
                <code className="bg-gray-100 px-1 rounded">
              ...
              <li>
                <strong>Enter below:</strong> Paste <strong>API URL</strong> and{" "}
                <strong>API Key</strong> in the form fields
              </li>
              <li>
                <strong>Note:</strong> Ensure provider supports
      ...
      default:
        return {
          ...
          content: (
            <p className="text-sm text-gray-600">
              Please select a provider type to see specific instructions.
            </p>
          ),
        };
    }
  ...
  const handleUpdateAIProvider = async () => {
    ...
    try {
      ...
      const current =
        currentSettings.data && currentSettings.data.length > 0
      ...
      const updateData = await apiService.updateSystemSettings({
        settings: updates,
      });
          ...
          <div className="space-y-4">
                ...
                {aiProviderForm.type && (
                  <div className="mt-3 p-3 bg-indigo-100 rounded border border-indigo-300">
                    <p className="text-xs text-indigo-800">
                      <strong>Quick Tip:</strong> After getting your
                      credentials, paste them in the form fields below and click
                      "Update AI Provider Settings" to save.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                API URL
                {aiProviderForm.type &&
                  ["groq", "openai", "anyscale"].includes(
                    aiProviderForm.type
                  ) && (
                    <span className="ml-2 text-xs text-gray-500 font-normal">
                      (Auto-filled, can be edited)
                    </span>
                  )}
              </label>
              <input
                ...
                placeholder={
                  aiProviderForm.type
                    ? "API URL will auto-populate"
                    : "Select provider type first"
                }
                value={aiProviderForm.api_url ?? ""}
                onChange={(e) =>
                  handleAIProviderChange("api_url", e.target.value)
                }
              />
              {aiProviderForm.type === "other" && (
                <p className="text-xs text-gray-500 mt-1">
                  Enter your custom API endpoint URL (e.g.,
                  https://api.provider.com/v1/chat/completions)
                </p>
              )}
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleAIProviderChange("api_key", e.target.value)
                }
              />
            </div>
            <div className="flex items-center mb-4">
              <input
                ...
                onChange={(e) =>
                  handleAIProviderChange("is_active", e.target.checked)
                }
                className="mr-2"
              />
          ...
          <div className="space-y-4">
            ...
            {showCloudinaryGuide && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                ...
                <ol className="list-decimal list-inside space-y-2 text-sm text-gray-700">
                  ...
                  <li>
                    <strong>Get credentials:</strong> Go to{" "}
                    <strong>Dashboard</strong> →{" "}
                    <strong>Account Details</strong> to find:
                    <ul className="list-disc list-inside ml-4 mt-1">
                      <li>
                        <strong>Cloud Name</strong> (top right of dashboard)
                      </li>
                      <li>
                        <strong>API Key</strong> (in Account Details)
                      </li>
                      <li>
                        <strong>API Secret</strong> (click "Show" to reveal)
                      </li>
                    </ul>
                  </li>
                  <li>
                    <strong>Copy & paste:</strong> Copy all three values and
                    paste them in the form fields below
                  </li>
                </ol>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleCloudinaryChange("cloudinary_api_key", e.target.value)
                }
              />
            </div>
            <div>
            ...
            <button
              className={`w-full py-3 bg-cyan-600 text-white font-bold rounded-lg transition-all ${
                !cloudinaryForm.cloudinary_cloud_name ||
                !cloudinaryForm.cloudinary_api_key ||
                !cloudinaryForm.cloudinary_api_secret ||
                loading
                  ? "opacity-50 cursor-not-allowed"
                  : "hover:bg-cyan-700"
              }`}
          ...
          <div className="space-y-4">
            ...
            {showGoogleOAuthGuide && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
                ...
                <ol className="list-decimal list-inside space-y-2 text-sm text-gray-700">
                  <li>
                    <strong>Go to Google Cloud Console:</strong> Visit{" "}
                    <a
                  ...
                  <li>
                    <strong>Create/Select Project:</strong> Create a new project
                    or select an existing one from the dropdown
                  </li>
                  <li>
                    <strong>OAuth Consent Screen:</strong> Go to{" "}
                    ...
                    <ul className="list-disc list-inside ml-4 mt-1">
                      ...
                      <li>
                        Fill app name, email, and add scopes:{" "}
                  ...
                  <li>
                    <strong>Copy Credentials:</strong> Copy{" "}
                    <strong>Client ID</strong> and{" "}
                    <strong>Client Secret</strong> from the popup and paste
                    below
                  </li>
                </ol>
                <div className="mt-3 p-2 bg-green-100 rounded border border-green-300">
                  <p className="text-xs text-green-800">
                    <strong>Security:</strong> Never commit Client Secret to
                    public repositories. Use environment variables.
                  </p>
                </div>
              </div>
            )}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleGoogleOAuthChange("google_client_id", e.target.value)
                }
              />
            </div>
            <div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleGoogleOAuthChange("google_redirect_uri", e.target.value)
                }
              />
            </div>
            <button
              className={`w-full py-3 bg-green-600 text-white font-bold rounded-lg transition-all ${
                !googleOAuthForm.google_client_id ||
                !googleOAuthForm.google_client_secret ||
                loading
                  ? "opacity-50 cursor-not-allowed"
                  : "hover:bg-green-700"
              }`}
