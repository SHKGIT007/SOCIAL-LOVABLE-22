      ...
      <span className="relative">
        ...
        <span
          className={`block w-11 h-6 rounded-full transition-colors duration-200 ${
            checked ? "bg-emerald-500/90" : "bg-gray-300"
          }`}
        />
        <span
          className={`absolute left-0 top-0 w-6 h-6 bg-white rounded-full shadow-md transform transition-transform duration-200 ${
...
export default function UserSchedules() {
  ...
  const handleRowChange = (idx, e) => {
    ...
    setRows((prev) => {
      ...
      if (name === "platforms") {
        ...
        if (checked) platforms.push(value);
        ...
        if (checked) {
          if (value === "daily") {
            ...
            if (!updated[idx].times[CUSTOM_DATE_KEY])
              updated[idx].times[CUSTOM_DATE_KEY] = [""];
            if (typeof updated[idx].customDateFrom === "undefined")
              updated[idx].customDateFrom = "";
            if (typeof updated[idx].customDateTo === "undefined")
              updated[idx].customDateTo = "";
          } else if (value === SINGLE_DATE_KEY) {
            days.push(SINGLE_DATE_KEY);
            if (!updated[idx].times[SINGLE_DATE_KEY])
              updated[idx].times[SINGLE_DATE_KEY] = [""];
            if (typeof updated[idx].singleDate === "undefined")
  ...
  const handleTimeChange = (rowIdx, day, timeIdx, value) => {
    setRows((prev) => {
      const updated = [...prev];
      const times = updated[rowIdx].times[day]
        ? [...updated[rowIdx].times[day]]
        : [""];
      times[timeIdx] = value;
  ...
  const handleRemoveTime = (rowIdx, day, timeIdx) => {
    setRows((prev) => {
      ...
      if (updated[rowIdx].times[day].length > 1) {
        updated[rowIdx].times[day] = updated[rowIdx].times[day].filter(
          (_, i) => i !== timeIdx
        );
      }
      return updated;
  ...
  const handleSubmit = async (e) => {
    ...
    for (let i = 0; i < rows.length; i++) {
      if (!rows[i].platforms || rows[i].platforms.length === 0) {
        Swal.fire(
          "Validation Error",
          "Please select at least one platform for every schedule.",
          "warning"
        );
        return;
      }
      if (!rows[i].days || rows[i].days.length === 0) {
        Swal.fire(
          "Validation Error",
          "Please select at least one day, custom date, or single date for every schedule.",
          "warning"
        );
        return;
      }
      if (rows[i].days.includes(CUSTOM_DATE_KEY)) {
        if (!rows[i].customDateFrom) {
          Swal.fire(
            "Validation Error",
            'Please select the "From" date before selecting "To" date for Custom Date Range.',
            "warning"
          );
          return;
        }
        if (!rows[i].customDateTo) {
          Swal.fire(
            "Validation Error",
            'Please select the "To" date for Custom Date Range.',
            "warning"
          );
          return;
        }
    ...
    try {
      if (editingId !== null) {
        ...
        // Compare old vs new
        if (!isScheduleChanged(original, updated)) {
          Swal.fire(
            "No Changes",
        ...
        await apiService.updateSchedule(editingId, updated);
        ...
        const result = await Swal.fire({
          ...
          text: "Do you want to create these schedule(s)?",
          icon: "warning",
          showCancelButton: true,
    ...
    setRows([
      {
        platforms: Array.isArray(schedule.platforms)
          ...
          ? schedule.platforms.split(",").map((p) => p.trim())
          : [],
        days: schedule.days || [],
          ...
          <div className="flex items-center justify-center min-h-screen px-4">
            ...
            <Dialog.Panel className="relative bg-white rounded-2xl shadow-2xl max-w-4xl w-full h-[90vh] mx-auto p-8 z-10 flex flex-col overflow-y-auto">
              <Dialog.Title className="text-3xl font-extrabold mb-1 flex items-center gap-3 text-indigo-700">
                ...
                <span className="text-gray-900">Post Schedules</span>
              </Dialog.Title>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                {rows.map((row, idx) => (
                  <div
                    ...
                    } p-4 md:p-5 shadow-sm bg-white`}
                  >
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="text-lg font-semibold text-gray-800">
                        Schedule Block {idx + 1}
                      </h4>
                    ...
                    {row.days.map((day) => (
                      <div
                        ...
                        className={`mb-3 ${
                          day === CUSTOM_DATE_KEY
                            ? "bg-indigo-50/60 border border-indigo-100 rounded-xl p-3"
                            : ""
                        }`}
                      >
