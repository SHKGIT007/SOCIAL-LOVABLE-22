...
const ToggleSwitch = ({ checked, onChange }: { checked: boolean; onChange: () => void }) => (
  <label className="inline-flex items-center cursor-pointer relative">
    ...
    <span className={`w-11 h-6 rounded-full transition-colors ${checked ? "bg-green-400" : "bg-gray-300"}`} />
    <span
      className={`absolute left-0 top-0 w-6 h-6 bg-white border rounded-full shadow transform transition-transform ${
...
export default function UserSchedules() {
  ...
  const handleRowChange = useCallback(
    (idx: number, e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const r of rows) {
      ...
      if (!r.days.length) {
        Swal.fire("Validation Error", "Please select at least one day, custom date, or single date.", "warning");
        return;
      }
    }
    ...
    setRows([
      {
        platforms: Array.isArray(sch.platforms)
          ? sch.platforms
          : typeof sch.platforms === "string"
          ? sch.platforms.split(",").map((p: string) => p.trim())
          : [],
        days: sch.days || [],
        times: sch.times || {},
        ...
        <div className="w-full">
          ...
          <div className="flex items-center justify-between mb-8">
            <h2 className="flex items-baseline gap-2">
              ...
              <span className="text-3xl font-extrabold text-gray-800 leading-none">Post Schedules</span>
            </h2>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-indigo-100 min-h-[350px]">
            <div className="space-y-4">
              ...
              {schedules.map((sch) => {
                ...
                // platforms into array
                const platformsArr = Array.isArray(sch.platforms)
                  ? sch.platforms
                  : typeof sch.platforms === "string"
                  ? sch.platforms
                      .split(",")
                ...
                return (
                  <div
                    key={sch.id}
                    className={`rounded-2xl border p-5 shadow-[0_8px_24px_-8px_rgba(0,0,0,0.15)] ${
                      isActive ? "bg-white border-indigo-200" : "bg-white border-gray-200"
                    }`}
                  >
                    {/* Row Header */}
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="truncate">
                          <div className="text-base font-semibold text-gray-800">Schedule</div>
                          <div className="text-xs text-gray-500 truncate">
                            {sch.recurrence ? `Recurs: ${sch.recurrence}` : "Custom schedule"}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-3">
                        {platformsArr.length === 0 ? (
                          ...
                          platformsArr.map((p: string) => (
                            <div
                              key={p}
                              className="flex items-center gap-3 px-4 py-3 bg-white border border-indigo-100 rounded-xl shadow-sm hover:shadow-md transition"
                            >
                              <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-indigo-500 to-sky-400 text-white grid place-items-center font-bold uppercase">
                                {p?.[0] || "P"}
                              </div>
                              <div>
          ...
          <div className="flex items-center justify-center min-h-screen px-4">
            ...
            <Dialog.Panel className="relative bg-white rounded-2xl shadow-2xl max-w-4xl w-full h-[95vh] mx-auto p-8 z-10 flex flex-col overflow-y-auto transform transition-all">
              <Dialog.Title className="text-3xl font-extrabold mb-6 flex items-center gap-3 text-indigo-800 border-b pb-3">
                ...
                <span className="bg-gradient-to-r from-indigo-600 to-sky-400 bg-clip-text text-transparent">Auto</span>
              ...
              <form onSubmit={handleSubmit} className="space-y-6 flex-1 overflow-y-auto pr-2">
                {rows.map((row, idx) => (
                  <div
                    key={idx}
                    className={`p-6 rounded-xl border border-indigo-200 bg-white shadow-lg transition-all duration-300 ${
                      idx > 0 ? "mt-6" : ""
                    }`}
                  >
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="flex gap-2">
                        {rows.length > 1 && (
                          <button
                            type="button"
                            className="bg-red-500 text-white rounded-full p-2 h-9 w-9 flex items-center justify-center shadow hover:bg-red-600 transition"
                        ...
                        <button
                          type="button"
                          className="bg-green-500 text-white rounded-full p-2 h-9 w-9 flex items-center justify-center shadow hover:bg-green-600 transition"
                          onClick={handleAddRow}
                          title="Add a new schedule block"
                        >
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2 mb-4 p-3 bg-indigo-50 rounded-lg border border-indigo-200">
                          {DAYS.map((day) => (
                            <label
                              key={day.value}
                              className={`px-4 py-2 text-xs font-bold rounded-full cursor-pointer transition-all duration-200 ${
                                row.days.includes(day.value)
                                  ? "bg-indigo-600 text-white shadow-lg"
                                  : "bg-white text-indigo-700 border border-indigo-300 hover:bg-indigo-100"
                              } ${day.value !== "daily" && row.days.includes("daily") ? "opacity-50 cursor-not-allowed" : ""}`}
                            >
                              <input
                                type="checkbox"
                                name="days"
                        ...
                        <div className="flex flex-wrap gap-4 items-center p-3 border-t border-gray-200 pt-4">
                          {/* Custom Range */}
                          <label
                            className={`flex items-center gap-2 transition p-2 rounded ${
                              row.days.includes(CUSTOM_DATE_KEY) ? "bg-orange-50" : "hover:bg-gray-50"
                            }`}
                          >
                            <input
                              type="checkbox"
                              ...
                              onChange={(e) => handleRowChange(idx, e)}
                              disabled={row.days.includes("daily") || row.days.includes(SINGLE_DATE_KEY)}
                          ...
                          {/* Single Date */}
                          <label
                            className={`flex items-center gap-2 transition ml-6 p-2 rounded ${
                              row.days.includes(SINGLE_DATE_KEY) ? "bg-orange-50" : "hover:bg-gray-50"
                            }`}
                          >
                            <input
                              type="checkbox"
                              ...
                              onChange={(e) => handleRowChange(idx, e)}
                              disabled={row.days.includes("daily") || row.days.includes(CUSTOM_DATE_KEY)}
                              className="form-checkbox text-indigo-600 rounded"
                            />
                            ...
                            <div className="flex justify-between items-center gap-2 mb-3">
                              <span
                                className={`text-sm font-bold flex items-center gap-2 ${
                                  day === CUSTOM_DATE_KEY || day === SINGLE_DATE_KEY ? "text-orange-700" : "text-indigo-700"
                                }`}
                              >
                                <Clock className="w-4 h-4" />
                                {day === CUSTOM_DATE_KEY
                                  ? `Custom Date${
                                      row.customDateFrom && row.customDateTo
                                        ? ` (${row.customDateFrom} to ${row.customDateTo})`
                                        : ""
                                    }`
                                  : day === SINGLE_DATE_KEY
                              ...
                              <button
                                type="button"
                                className="bg-indigo-100 text-indigo-600 text-xs font-semibold px-3 py-1 rounded-full hover:bg-indigo-200 transition"
                              ...
                              {(row.times[day] || []).map((time, tIdx) => (
                                <div key={tIdx} className="flex items-center p-1 bg-white border border-indigo-300 rounded-lg shadow-sm">
                                  <input
                                    ...
                                    className="p-1 text-sm border-none focus:ring-0 rounded"
                                    required
                                  />
                ...
                <div className="flex gap-4 mt-8 pt-4 border-t border-gray-200">
                  <button
                    type="submit"
                    className="bg-indigo-600 text-white px-8 py-3 rounded-xl shadow-lg hover:bg-indigo-700 font-bold transition transform hover:scale-[1.01]"
                  >
                    {editingId ? "Update" : "Create"} Schedule{rows.length > 1 ? "s" : ""}
                  </button>
                  <button
                    type="button"
                    className="px-6 py-3 rounded-xl border border-gray-300 bg-gray-50 text-gray-700 hover:bg-gray-100 transition font-semibold"
                    onClick={() => {
                      setRows([emptyRow()]);
                      setEditingId(null);
                      setModalOpen(false);
